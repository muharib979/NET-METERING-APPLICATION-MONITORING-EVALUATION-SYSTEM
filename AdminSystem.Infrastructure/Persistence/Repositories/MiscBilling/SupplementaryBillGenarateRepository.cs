...
{
    public class SupplementaryBillGenarateRepository : ISupplementaryGenarateRepository
    {
        private readonly IMapper _mapper;
        private readonly string _billnumber;
        public SupplementaryBillGenarateRepository(IMapper mapper)
        {
            _mapper = mapper;
        }

        public async Task<List<SupplementaryBillViewDTO>> GetSupplementaryView(SupplementaryBillViewDTO model)
        {
            ...
            var result = con.Query<SupplementaryBillViewDTO>("DPG_MISCBILL_PENALTY_BILL_GEN.GET_CUSTOMER_SUPLIMENTARY_BILL", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure).ToList();
            return result;
        }

        public async Task<bool> SaveSupplementaryExistingConsumer(SupplementaryExistingConsumerDTO model)
        {
            bool isSuccess = true;
            using var con = new OracleConnection(Connection.ConnectionString());
            OracleDynamicParameters oracleDynamicParameters = new OracleDynamicParameters();
            ...
            oracleDynamicParameters.Add("P_BILL_TYPE_CODE", value: "2", dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
            ...
            var result = await con.ExecuteAsync("DPG_MISCBILL_SUPPLMNTR_BILL.DPD_SUPPLEMENTARY_EXISTING_BILL_SAVE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
            isSuccess = oracleDynamicParameters.Get<int>("O_Status") > 0;
            return (isSuccess);
        }

        public async Task<ReturnDto> SaveSupplenmentaryBill(SupplementaryBillDTO model)
        {
            try
            {
                ...
                oracleDynamicParameters.Add("P_DUE_DATE", value: model.DueDate, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                oracleDynamicParameters.Add("p_REMARK", value: model.Remarks, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                ...
                oracleDynamicParameters.Add("P_BILL_REASON_CODE", value: model.BillReasonCode, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                oracleDynamicParameters.Add("P_UNIT_FIXED", value: model.MultypliBy, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                ...
                if (model.BillNumber != null)
                {
                    ...
                    result = con.QuerySingle<ReturnDto>("DPG_MISCBILL_SUPLMNTR_BILL_RGN.DPD_SUPP_BILL_SAVE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
                    string quesry = $"DELETE FROM MISCBILL_BILL_MST_FILE WHERE BILL_NUMBER ={model.BillNumber}";
                    con.Execute(quesry);
                }
                else
                {
                    result = con.QuerySingle<ReturnDto>("DPG_MISCBILL_SUPPLMNTR_BILL.DPD_SUPP_BILL_SAVE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
