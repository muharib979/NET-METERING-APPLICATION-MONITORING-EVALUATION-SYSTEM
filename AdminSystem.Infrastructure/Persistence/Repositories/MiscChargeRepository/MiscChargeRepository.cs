...
namespace AdminSystem.Infrastructure.Persistence.Repositories.MiscChargeRepository
{
    public class MiscChargeRepository : IMiscChargeRepository
    {
        private readonly IMapper _mapper;

        public MiscChargeRepository(IMapper mapper)
        {
            _mapper = mapper;
        }
        ...
        public async Task<List<DcTypeDTO>> GetAllDcType()
        {
            try
            {
                ...
                var result = con.Query<DcType>("DPG_MISCBILL_CHARGE_CUST.DPD_DC_TYPE_LIST", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure).ToList();
        ...
        public async Task<List<MiscChargeViewDTO>> GetMiscView(MiscChargeViewDTO model)
        {
            using var con = new OracleConnection(Connection.ConnectionString());
            OracleDynamicParameters oracleDynamicParameters = new OracleDynamicParameters();
            oracleDynamicParameters.Add(":cur_data", dbType: OracleMappingType.RefCursor, direction: ParameterDirection.Output);
            ...
            var result = con.Query<MiscChargeViewDTO>("DPG_MISCBILL_PENALTY_BILL_GEN.GET_CUSTOMER_MISC_CHARGE_BILL", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure).ToList();
            return result;
        }

        public async Task<ReturnDTO> SaveMiscChargeBill(MiscChargeDTO model)
        {
            try
            {
                ...
                oracleDynamicParameters.Add("P_USER", value: model.P_USER, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                ...
                oracleDynamicParameters.Add("p_xformer_kva", value: model.Xformer_kva, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                oracleDynamicParameters.Add("p_xformer_day_rent", value: model.Xformer_day_rent, dbType: (OracleMappingType?)OracleDbType.Varchar2, direction: ParameterDirection.Input);
                ...
                if (model.BillNumber != null)
                {
                    ...
                    result = con.QuerySingle<ReturnDTO>("DPG_MISCBILL_CHARGE_CUST_RGN.DPD_DCRC_BILL_SAVE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
                ...
                {
                    result = con.QuerySingle<ReturnDTO>("DPG_MISCBILL_CHARGE_CUST.DPD_DCRC_BILL_SAVE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
                }
        ...
        public async  Task<bool> UpdateMiscDate(string billNumber, string customerNumber, string rcDate)
        {
            ...
            var result = con.Query<int>("DPG_MISCBILL_CHARGE_CUST.DPD_DCRC_BILL_STATUS_UPDATE", param: oracleDynamicParameters, commandType: CommandType.StoredProcedure);
