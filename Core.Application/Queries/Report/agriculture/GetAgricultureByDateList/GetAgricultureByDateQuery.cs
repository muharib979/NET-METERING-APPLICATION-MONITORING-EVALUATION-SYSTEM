...
{
    public class GetAgricultureByDateQuery : IRequest<Response<IEnumerable<AgricultureDto>>>
    {
        public string ValidDate { get; set; }

        public class Handler : IRequestHandler<GetAgricultureByDateQuery, Response<IEnumerable<AgricultureDto>>>
        {
            ...
            public async Task<Response<IEnumerable<AgricultureDto>>> Handle(GetAgricultureByDateQuery request, CancellationToken cancellationToken)
            {
                var result = await _repository.GetAllAgricultureAsyncByDate(request.ValidDate);
